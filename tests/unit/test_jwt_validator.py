...
def test_get_jwks_failure_raises_typed_error(monkeypatch):
    metric = Mock()
    monkeypatch.setattr(
    ...
    assert error.value.context["failure_category"] == "request_failed"
    assert error.value.context["cause_type"] == "Timeout"
